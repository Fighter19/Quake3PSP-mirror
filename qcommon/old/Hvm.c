...
#error vm not available -> only static
   ...
   {
    Com_Printf("VM UI\n");
		extern int vmMainUI ( int callNum, ... );
		vm->entryPoint = vmMainUI;
		...
		dllEntryUI(VM_DllSyscall);
   }
   else if(module == "cgame")
   {
    Com_Printf("VM CGAME\n");
	  extern int CGvmMain ( int callNum, ... );
		vm->entryPoint = CGvmMain;
		Com_Printf("VM_Create(%s) found **vmMain** at  %p  \n",module,CGvmMain);
		vm->dllHandle = (void *) 1; //some strange but works :)
		void CGdllEntry( int (QDECL *syscallptr)( int arg,... ) ) ;

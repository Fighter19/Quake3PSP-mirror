...
#define DIE( reason )                                                 \
	do {                                                              \
		printf("vm_sparc compiler error: " reason "\n");              \
		Com_Printf("vm_sparc compiler error: " reason "\n");          \
...
static void vimm(unsigned int val, int bits, int shift, int sgned, int arg_index)
{
	...
	if (val & ~((1U << bits) - 1U))
	{
		printf("VM ERROR: immediate value 0x%08x out of %d bit range\n",
			   orig_val, orig_bits);
		DIE("sparc VM bug");
	}
...
*/
    data_and_code = malloc(code_length);
...
/*
  byte* n;
  n = realloc(vm->codeBase,vm->codeLength);
  if (!n) 
	{

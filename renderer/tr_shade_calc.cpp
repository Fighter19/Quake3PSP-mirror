...
static void GlobalVectorToLocal( const vec3_t in, vec3_t out ) {
	out[0] = DotProduct( in, backEnd.por.axis[0] );
	out[1] = DotProduct( in, backEnd.por.axis[1] );
	out[2] = DotProduct( in, backEnd.por.axis[2] );
}
...
static void AutospriteDeform( void ) {
	...
	if ( backEnd.currentEntity != &tr.worldEntity ) {
		GlobalVectorToLocal( backEnd.viewParms.por.axis[1], leftDir );
		GlobalVectorToLocal( backEnd.viewParms.por.axis[2], upDir );
	} else {
		VectorCopy( backEnd.viewParms.por.axis[1], leftDir );
		VectorCopy( backEnd.viewParms.por.axis[2], upDir );
	}
...
static void Autosprite2Deform( void ) {
	...
	if ( backEnd.currentEntity != &tr.worldEntity ) {
		GlobalVectorToLocal( backEnd.viewParms.por.axis[0], forward );
	} else {
		VectorCopy( backEnd.viewParms.por.axis[0], forward );
	}
...
void RB_CalcFogTexCoords( float *st ) {
	...
	VectorSubtract( backEnd.por.origin, backEnd.viewParms.por.origin, local );
	fogDistanceVector[0] = -backEnd.por.modelMatrix[2];
	fogDistanceVector[1] = -backEnd.por.modelMatrix[6];
	fogDistanceVector[2] = -backEnd.por.modelMatrix[10];
	fogDistanceVector[3] = DotProduct( local, backEnd.viewParms.por.axis[0] );
	...
	if ( fog->hasSurface ) {
		fogDepthVector[0] = fog->surface[0] * backEnd.por.axis[0][0] +
			fog->surface[1] * backEnd.por.axis[0][1] + fog->surface[2] * backEnd.por.axis[0][2];
		fogDepthVector[1] = fog->surface[0] * backEnd.por.axis[1][0] +
			fog->surface[1] * backEnd.por.axis[1][1] + fog->surface[2] * backEnd.por.axis[1][2];
		fogDepthVector[2] = fog->surface[0] * backEnd.por.axis[2][0] +
			fog->surface[1] * backEnd.por.axis[2][1] + fog->surface[2] * backEnd.por.axis[2][2];
		fogDepthVector[3] = -fog->surface[3] + DotProduct( backEnd.por.origin, fog->surface );

		eyeT = DotProduct( backEnd.por.viewOrigin, fogDepthVector ) + fogDepthVector[3];
	} else {
...
void RB_CalcEnvironmentTexCoords( float *st ) 
{
	...
	{
		VectorSubtract (backEnd.por.viewOrigin, v, viewer);

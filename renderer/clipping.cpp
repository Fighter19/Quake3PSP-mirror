...
namespace quake
{
	namespace clipping
	{
		...
		static const std::size_t	max_clipped_vertices	= 2000;//32;
		...
		void ColorClampf(float r, float g, float b, float a)
		{
            //0 - 1
			if(r > 1) r = 1;
			if(r < 0) r = 0;
		...
		void ColorClampi(float r, float g, float b, float a)
		{
            //0 - 255
...
*/
			...
			delta.st[0]  = v2.st[0]  - v1.st[0];
            delta.st[1]  = v2.st[1]  - v1.st[1];

            r[0] = (v2.c & 0xff);
			g[0] = (v2.c & 0xff00)     >>8;
            b[0] = (v2.c & 0xff0000)   >>16;
            a[0] = (v2.c & 0xff000000) >>24;

		    r[1] = (v1.c & 0xff);
			g[1] = (v1.c & 0xff00)     >>8;
            b[1] = (v1.c & 0xff0000)   >>16;
            a[1] = (v1.c & 0xff000000) >>24;
			...
			//Clamp
			ColorClampi(r[2], g[2], b[2], a[2]);
            
            //delta.c = GU_RGBA((int)r[2], (int)g[2], (int)b[2], (int)a[2]);

			delta.xyz[0] = v2.xyz[0] - v1.xyz[0];
            delta.xyz[1] = v2.xyz[1] - v1.xyz[1];
            delta.xyz[2] = v2.xyz[2] - v1.xyz[2];
...
*/
            v.st[0]  = v1.st[0]  + time * delta.st[0];
            v.st[1]  = v1.st[1]  + time * delta.st[1];
/*
		    r[0] = (delta.c & 0xff);
			g[0] = (delta.c & 0xff00)     >>8;
            b[0] = (delta.c & 0xff0000)   >>16;
            a[0] = (delta.c & 0xff000000) >>24;
*/
		    r[1] = (v1.c & 0xff);
			g[1] = (v1.c & 0xff00)     >>8;
            b[1] = (v1.c & 0xff0000)   >>16;
			...
			//delta
			r[2] = (r[1] / 255.0f) + time * (r[2] / 255.0f);
			g[2] = (g[1] / 255.0f) + time * (g[2] / 255.0f);
			b[2] = (b[1] / 255.0f) + time * (b[2] / 255.0f);
			a[2] = (a[1] / 255.0f) + time * (a[2] / 255.0f);

			//Clamp
	        ColorClampf(r[2], g[2], b[2], a[2]);

            v.c = GU_RGBA((int)(r[2] * 255.0f),
			              (int)(g[2] * 255.0f),
						  (int)(b[2] * 255.0f),
						  (int)(a[2] * 255.0f));

		    v.xyz[0] = v1.xyz[0] + time * delta.xyz[0];
            v.xyz[1] = v1.xyz[1] + time * delta.xyz[1];
            v.xyz[2] = v1.xyz[2] + time * delta.xyz[2];

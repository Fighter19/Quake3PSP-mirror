...
static	void R_LoadLightmaps( lump_t *l )
{
	...
	image = (byte*)ri.Malloc((LIGHTMAP_SIZE * LIGHTMAP_SIZE * 4) * sizeof(byte));
	...
	for ( i = 0 ; i < tr.numLightmaps ; i++ )
	{
		...
		tr.lightmaps[i] = R_CreateImage( va("*lightmap%d",i), image,
			LIGHTMAP_SIZE, LIGHTMAP_SIZE, qfalse, qfalse, GU_CLAMP );
...
int R_MergedWidthPoints(srfGridMesh_t *grid, int offset)
{
	...
	for (i = 1; i < grid->width-1; i++) {
		...
		{
#ifdef PSP_VFPU
            if ( vfpu_fabsf(grid->verts[i + offset].xyz[0] - grid->verts[j + offset].xyz[0]) > .1) continue;
			if ( vfpu_fabsf(grid->verts[i + offset].xyz[1] - grid->verts[j + offset].xyz[1]) > .1) continue;
			if ( vfpu_fabsf(grid->verts[i + offset].xyz[2] - grid->verts[j + offset].xyz[2]) > .1) continue;
#else
...
int R_MergedHeightPoints(srfGridMesh_t *grid, int offset)
{
	...
	for (i = 1; i < grid->height-1; i++)
	{
		...
		{
#ifdef PSP_VFPU
            if ( vfpu_fabsf(grid->verts[grid->width * i + offset].xyz[0] - grid->verts[grid->width * j + offset].xyz[0]) > .1) continue;
			if ( vfpu_fabsf(grid->verts[grid->width * i + offset].xyz[1] - grid->verts[grid->width * j + offset].xyz[1]) > .1) continue;
			if ( vfpu_fabsf(grid->verts[grid->width * i + offset].xyz[2] - grid->verts[grid->width * j + offset].xyz[2]) > .1) continue;
#else
		...
		for (n = 0; n < 2; n++) {
			...
			for (k = 1; k < grid1->width-1; k++) {
				for (m = 0; m < 2; m++) {
					...
					//
#ifdef PSP_VFPU
                        if ( vfpu_fabsf(grid1->verts[k + offset1].xyz[0] - grid2->verts[l + offset2].xyz[0]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[k + offset1].xyz[1] - grid2->verts[l + offset2].xyz[1]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[k + offset1].xyz[2] - grid2->verts[l + offset2].xyz[2]) > .1) continue;
#else
				...
				for (m = 0; m < 2; m++) {
					...
					//
#ifdef PSP_VFPU
                        if ( vfpu_fabsf(grid1->verts[k + offset1].xyz[0] - grid2->verts[grid2->width * l + offset2].xyz[0]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[k + offset1].xyz[1] - grid2->verts[grid2->width * l + offset2].xyz[1]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[k + offset1].xyz[2] - grid2->verts[grid2->width * l + offset2].xyz[2]) > .1) continue;
#else
		...
		for (n = 0; n < 2; n++) {
			...
			for (k = 1; k < grid1->height-1; k++) {
				for (m = 0; m < 2; m++) {
					...
					//
#ifdef PSP_VFPU
						if ( vfpu_fabsf(grid1->verts[grid1->width * k + offset1].xyz[0] - grid2->verts[l + offset2].xyz[0]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[grid1->width * k + offset1].xyz[1] - grid2->verts[l + offset2].xyz[1]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[grid1->width * k + offset1].xyz[2] - grid2->verts[l + offset2].xyz[2]) > .1) continue;
#else
				...
				for (m = 0; m < 2; m++) {
					...
					//
#ifdef PSP_VFPU
						if ( vfpu_fabsf(grid1->verts[grid1->width * k + offset1].xyz[0] - grid2->verts[grid2->width * l + offset2].xyz[0]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[grid1->width * k + offset1].xyz[1] - grid2->verts[grid2->width * l + offset2].xyz[1]) > .1) continue;
						if ( vfpu_fabsf(grid1->verts[grid1->width * k + offset1].xyz[2] - grid2->verts[grid2->width * l + offset2].xyz[2]) > .1) continue;
#else
		...
		for (k = 0; k < grid1->width-2; k += 2) {
			...
			for (m = 0; m < 2; m++) {
				...
				//
...
#ifdef PSP_VFPU
	                if ( vfpu_fabsf(v1[0] - v2[0]) > .1)
						continue;
					if ( vfpu_fabsf(v1[1] - v2[1]) > .1)
						continue;
					if ( vfpu_fabsf(v1[2] - v2[2]) > .1)
						continue;
#else
					if ( fabs(v1[0] - v2[0]) > .1)
						continue;
					...
					v2 = grid2->verts[l + 1 + offset2].xyz;
#ifdef PSP_VFPU
                    if ( vfpu_fabsf(v1[0] - v2[0]) < .01 &&
							vfpu_fabsf(v1[1] - v2[1]) < .01 &&
							vfpu_fabsf(v1[2] - v2[2]) < .01)
					    continue;
#else
